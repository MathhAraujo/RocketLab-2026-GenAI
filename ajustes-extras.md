...
| 2026-04-19 | Backend captura `google.genai.errors.APIError` (lançada pelo SDK do Google para 429) além de `ModelHTTPError`: classifica por `exc.code == 429` + "quota" em `exc.details` → `GeminiQuotaExhaustedError` (503); sem "quota" → `GeminiRateLimitError` (429); ambas re-levantadas em vez de cair no fallback genérico | `backend/app/agents/sql_agent.py`, `backend/app/services/assistente_service.py`, `backend/app/errors.py` |
