...
### TASK-38 — Formatação contextual de células numéricas em `DynamicTable`

Arquivos: `frontend/src/utils/formatters.ts` (novo), `frontend/src/components/assistente/DynamicTable.tsx`.

- [x] Criar `frontend/src/utils/formatters.ts` com constantes `MONETARY_KEYWORDS`, `MAX_DECIMAL_PLACES`,
      formatador BRL via `Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' })`, e função
      exportada `formatCell(columnName: string, value: unknown): string`.
- [x] Heurística: coluna é monetária se `columnName.toLowerCase()` contém qualquer item de `MONETARY_KEYWORDS`
      (`preco`, `frete`, `receita`, `valor`, `brl`).
- [x] `formatCell` retorna: `'—'` para null/undefined; string original para não-numérico; BRL para monetário;
      `toLocaleString('pt-BR', { maximumFractionDigits: MAX_DECIMAL_PLACES })` para float genérico;
      `toLocaleString('pt-BR')` para inteiro.
- [x] Modificar `cellText` em `DynamicTable.tsx` para aceitar `columnName: string` e delegar a `formatCell`.
- [x] Manter `escapeCsvCell` intacto — CSV exporta valores originais sem formatação.
- [x] Rodar `npm run lint`, `npm run type-check`, `npm run format:check` — zero issues.
- [x] Verificar manualmente: `preco_BRL` exibe `R$ X,XX`; `avaliacao_media` exibe ≤ 4 casas; inteiros sem
      `.00`; CSV baixado contém o valor numérico bruto.
